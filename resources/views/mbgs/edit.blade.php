...
@section('content')
    <div
        class="max-w-7xl mx-auto px-6 py-8 bg-gradient-to-br from-green-100 via-blue-100 to-purple-100 rounded-xl shadow-lg">
        ...
        <form action="{{ route('mbgs.updateByDate') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <tbody>
                        @foreach ($classes as $class)
                            ...
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                <td>
                                    <input type="file" name="mbgs[{{ $class->id }}][foto]"
                                        class="text-sm border-2 border-blue-400 p-2 rounded-lg">
                                </td>
                                <td>
                                    <textarea name="mbgs[{{ $class->id }}][keteranganmbg]" class="border rounded p-2 w-full text-sm">{{ $mbg->keteranganmbg ?? '' }}</textarea>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
            ...
            <div class="mt-8 flex justify-end gap-4 flex-wrap">
                <button type="button" onclick="window.location.href='{{ route('mbgs.index') }}'"
